...
from src.geometry import count_receptor_combinations
...
ARMS = [
    ...
    ("ng5", "n_receptors", 5,    "$n_g=5$",   "#1f77b4", "-" ),
    ("ng3", "n_receptors", 3,    "$n_g=3$",   "#ff7f0e", "-"),
]
...
for prefix, r_col, n_genes_fixed, label, color, ls in ARMS:
    ...
    if prefix != "homomers":
        max_count = count_receptor_combinations(n_genes_fixed, k_sub=5,use_interface_model=False)
        print(max_count)
        ax.vlines(x=max_count, ymin=0, ymax=max_count)

ax.plot(np.arange(1, 40, 1), np.arange(1, 40, 1),
