...
# First-shot heteromer arm: uniform_random strategy, n_genes = 5 (§6, narrative_and_next_steps.md).
# Receptor fan-out warm-start: each n_receptors > 5 branches from the (n_genes=5, n_receptors=5) baseline.
# Environment axis: n_ligands × average_family_distance.  D = 10 fixed.
#
# docker compose -f /home/leroy/opt_bin_resp/docker-compose.server.yaml run --rm gpu-runner python3 /app/run/script/Fig1_first_shot/first_shot_heteromers_rand_ng5.py
...
m, s = divmod(rem, 60)
print(f"\nHeteromer uniform_random ng=5 sweep complete!  {int(h)}h {int(m)}m {s:.0f}s")
