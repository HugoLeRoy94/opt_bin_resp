...
# First-shot heteromer arm: cascading strategy, n_genes = 8 (§6, narrative_and_next_steps.md).
# Warm-starts over n_receptors from 8 to 15; n_genes fixed.
# Environment axis: n_ligands × average_family_distance.  D = 10 fixed.
#
# docker compose -f /home/leroy/opt_bin_resp/docker-compose.server.yaml run --rm gpu-runner python3 /app/run/script/Fig1_first_shot/first_shot_heteromers_casc_ng8.py
...
m, s = divmod(rem, 60)
print(f"\nHeteromer cascading ng=8 sweep complete!  {int(h)}h {int(m)}m {s:.0f}s")
