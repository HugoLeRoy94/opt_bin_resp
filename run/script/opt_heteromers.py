            ...
            for n_units in n_units_list:
                for n_receptors in n_receptors_list:
                    ...
                    if n_receptors > max_combinations:
                        print(f"Breaking: n_receptors={n_receptors} exceeds max possible combinations ({max_combinations}) for n_units={n_units}")
                        break
                    ...
                    if len(already_computed_dirs) > 0:
                        print(f"Skipping: Families={n_families}, Dim={latent_dim}, Units={n_units}, Receptors={n_receptors}, Sample={sample+1}/{n_samples} (Already exists)")
