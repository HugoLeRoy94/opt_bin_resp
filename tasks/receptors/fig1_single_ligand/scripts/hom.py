#!/usr/bin/env python3

# Homomers sweep: n_genes from 3 to 49 with warm-start chain on n_genes.
# R = n_genes (one homomer per gene).
# 50 runs with random parameters sampled within the high-entropy regime
# (rho in [0.2,1], d_fam/lambda in [0.5,1.5], R_eff=2*mu_lig > R_max=49).
#
# docker compose -f /mnt/hcleroy/PostDoc2/octopus_smelling/opt_bin_resp/docker-compose.yaml run --rm gpu-runner python3 /app/scripts/single_ligand/hom.py

import time
import numpy as np
import sys
sys.path.append('/app')

from src.config import RunConfig
from src.run import SweepRunner

N_RUNS = 5                             # independent samples (different seeds)
_SWEEP = list(range(1, 11))            # n_genes values, length 10
_NS    = len(_SWEEP)
_FAM   = [.01, 0.05, 0.1, 0.2, 0.5]    # family_spread values, length 5
_NF    = len(_FAM)

# One cartesian block = n_genes (inner) x family_spread (outer), length _NS*_NF.
# Replicating the block N_RUNS times yields N_RUNS independent env realizations
# per (n_genes, family_spread) cell (the global RNG advances run-to-run).
_BLK_NG  = _SWEEP * _NF
_BLK_FAM = np.repeat(_FAM, _NS).tolist()
_BLK_EP  = [int(170 * n + 500) for n in _SWEEP] * _NF


config = RunConfig(
    # --- Environment ---
    n_families              = 1,
    n_ligands               =200,
    latent_dim              = 5,
    family_spread           = _BLK_FAM * N_RUNS,
    average_family_distance = 1.,
    environment_geometry    = "asymmetric",
    distribution_type       = "gaussian",
    observation_noise_sigma = 0.01,
    initial_temperature="auto",

    # --- Presence (hierarchical sampler) ---
    n_presence_blocks      = 1,
    mu_sources             = 1,
    mu_ligands_per_source  = 1,
    block_shared_conc_mean = False,

    # --- Interface model ---
    use_interface_model = False,


    # --- Concentration ---
    conc_model_type = "lognormal",
    conc_mean       = -5,
    conc_std        = 1,

    # --- Physics ---
    k_sub=5, temperature=0.1, affinity_kernel="gaussian", kernel_params=(1.0,),

    # --- Loss ---
    entropy="annealed",
    # --- Training ---
    epochs=_BLK_EP * N_RUNS, lr=0.01, use_scheduler=False,
    batch_size="auto", test_batch_size="auto",
    measurement_fns=("full_array_entropy",),

    # --- Sweep ---
    n_genes     = _BLK_NG * N_RUNS,
    sweep_name  = "homomers",
    base_folder = "/app/data/fig1_single_ligand",
    warm_start  = False,
)

print(config)
t0 = time.time()
SweepRunner(config).execute()
h, rem = divmod(time.time() - t0, 3600)
m, s = divmod(rem, 60)
print(f"\nHomomer sweep complete!  {int(h)}h {int(m)}m {s:.0f}s")
